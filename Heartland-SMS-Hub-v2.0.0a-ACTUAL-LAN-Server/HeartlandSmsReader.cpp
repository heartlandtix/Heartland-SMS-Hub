...
static std::wstring WriteOutputs(
    const std::vector<SmsEventSink::RawMessage>& messages)
{
...
int wmain()
{
    ...
    std::wstring outputStem = WriteOutputs(currentMessages);

    HttpServer webServer;
    const unsigned short webPort = 8080;
    const std::wstring inboxPath = outputStem + L".html";

    if (!webServer.Start(webPort, inboxPath))
    {
        std::wcerr << L"Could not start the LAN web server on port "
                   << webPort << L".\n";
        CoUninitialize();
        return 8;
    }
    ...
    std::wcout << L"Local inbox: http://localhost:8080/\n";
    std::wcout << L"LAN inbox:   http://<this-PC-IP>:8080/\n";
    std::wcout << L"Health test: http://localhost:8080/health\n";
